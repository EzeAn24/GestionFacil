...
class PestañaVentasGeneral:
    ...
    def construir(self):
        ...
        ctk.CTkLabel(self.marco_v, text="Venta - Almacén General", font=("Roboto", 16, "bold")).grid(row=0, column=0, columnspan=6, pady=5)
        ...
        self.opt_venta_prod = ctk.CTkOptionMenu(self.marco_v, values=["No hay stock"], width=230)
        self.opt_venta_prod.grid(row=1, column=1, padx=5)
        
        self.opt_tipo_v = ctk.CTkOptionMenu(self.marco_v, values=["Unidad", "Pack"], width=90)
        self.opt_tipo_v.grid(row=1, column=2, padx=5)
        
        self.ent_cant_v = ctk.CTkEntry(self.marco_v, placeholder_text="Cantidad", width=90)
        ...
        ctk.CTkLabel(marco_hist, text="Libro Diario General", font=("Roboto", 16, "bold")).pack(side="left")
    ...
    def filtrar_productos(self, event=None):
        ...
        filtrados = [p for p, info in self.app.mapa_productos.items() if texto in p.lower() and info['seccion'] == "General"]
        if filtrados:
    ...
    def actualizar_lista_productos(self):
        ...
        filtrados = [p for p, info in self.app.mapa_productos.items() if info['seccion'] == "General"]
        if filtrados:
            self.opt_venta_prod.configure(values=filtrados)
            self.opt_venta_prod.set(filtrados[0])
        else:
    ...
    def vender(self):
        ...
        try:
            ...
            tipo_v = self.opt_tipo_v.get()
            cant = float(self.ent_cant_v.get())
            pago = self.opt_pago.get()

            # Candado de Stock y Packs
            if tipo_v == "Pack" and info['precio_p'] == 0:
                messagebox.showerror("Error", f"El producto '{prod}' no se vende por pack.")
                return

            cant_desc = cant if tipo_v == "Unidad" else cant * info['u_pack']
            if cant_desc > info['stock']:
                messagebox.showerror("Error", f"Stock insuficiente. Quedan {info['stock']} físicas.")
                return

            if tipo_v == "Unidad":
                total = cant * info['precio_u']
                ganancia = cant * (info['precio_u'] - info['costo_u'])
            else:
                total = cant * info['precio_p']
                ganancia = cant * (info['precio_p'] - (info['costo_u'] * info['u_pack']))

            self.app.db.registrar_venta(caja[0], info['id'], prod, cant, tipo_v, pago, total, ganancia, "General")
            self.ent_cant_v.delete(0, 'end')
            self.ent_fecha.delete(0, 'end')
            self.ent_fecha.insert(0, datetime.now().strftime("%d/%m/%Y"))
            self.app.actualizar_todo()
        except ValueError: 
            messagebox.showerror("Error", "Ingresa una cantidad numérica válida.")
    ...
    def actualizar_historial(self):
        ...
        ventas = self.app.db.obtener_ventas(self.ent_fecha.get(), "General")
        
        if not ventas:
            ctk.CTkLabel(self.marco_hist_v, text="No hay ventas registradas en General.", font=("Roboto", 14, "italic")).pack(pady=20)
            return
        ...
        for v in ventas:
            ...
            cant_str = f"{int(cant_flotante)}" if cant_flotante.is_integer() else f"{cant_flotante:.2f}"
            
            ctk.CTkLabel(ren, text=f"📅 {fecha} | {cant_str} u/packs de {nom}", font=("Roboto", 13), width=350, anchor="w").pack(side="left")
